...
class Postgres {
    ...
    _createPool() {
        ...
        this.pool.on('error', (err) => {
            winston.error('Postgres pool error:', err);
            // Attempt to reconnect after 2 seconds
            setTimeout(() => {
                winston.info('Re-establishing Postgres connection...');
                this._createPool();
            }, 2000);
        });
    }
    ...
    connect() {
        return this.pool.connect().then((client) => {
            client.on('error', (err) => {
                winston.error('Postgres client error:', err);
                // Optionally, you can release the client or handle reconnection logic here
            });
            client.release(); // Release immediately after schema check
